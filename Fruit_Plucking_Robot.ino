...
int Step=4; //1=serial_data()&&motion(), 2 = pick_and_place() 
int look_out_var=0; 
float i=0.0;
bool Home_var= true;
...
void loop()
{if (Step==4)         // to aproach the tree 
    { motion(); 
      approach_tree(); }
 if (Tree_found==true)    
    { if (Step==1)         // to search for orange 
         {serial_data();
          motion(); }
      if (Step==2)         // to pick and place  
         {pick_and_place(); 
          //Move_robot(0); 
         }  
      if (Step==3)         // to look_out
         {look_out();
          Move_robot(0); }       
    }        
}     
...
{ 
  ...
  if (Serial_input==1 || Serial_input>2200 )
      { Step=3;
        look_out_time=millis();  
      }    
  if (Serial_input<2200 && Serial_input!=1)  //its orange then    
      { 
        if (Serial_input>1400) // its width then 
          { Width=round((Width*5/10)+(Serial_input*5/10)); 
            float error_1=abs(1800-Width);   // 1800 is desired value 
                if (Width<1800)
                   { Move=5;} // move_forward 
                if (Width<1700) // out of grabbing range 
                   { wait=1;}    
                if (wait==1)    // means do not grab , reset grab delay 
                   {wait_time=millis();}   
                if (Width>1800 && Speed_V<=1.4)  // means object is in right spot to grab
                   { Move=6;  // move back
                ...
                Speed_forward_reverse= speed_map(error_1,400,0,12.3,0.0);
          }
       if (Width<1700)
          { wait=1;}      
...
//                                  ///////////////////   move up down ////////////////
      if (Serial_input<800 && Serial_input>1)
         { error=abs(480-Serial_input); 
           ...
           Speed_H= speed_map(error,400,margin_with_center,8.5,0.0); 
         }    
...
{ if (EndY>0 && EndX>50) 
  ...
  if (EndY<0 && EndX>100) 
    { EndX=EndX-0.03; }  
  if (EndY<0 && ((EndX_memo-EndX)<60) && EndX<110) 
    { Move_robot(2); }  
  motion();             
}
...
{ if (Serial.available()>0)
  ...
  if ( diff>=2000 && diff<3000) 
     { retract(); } 
  if ( diff>=3000 && diff<4500)  // 2000 and 5000
     { place(); 
       Move_robot(0);
     } 
  if ( diff>=4500 && diff<5200)   // 5000 and 6000
     { Open();
       Move_robot(0);
     }   
  if ( diff>=5200 && diff<7500)    // 6000 and 10000 
     { Back_to_Home();
       Move_robot(0);
     }         
}
...
void look_out() 
{ Open(); 
 // Serial.println(millis()-look_out_time);
  if (Serial.available()>0)
     { String str= Serial.readStringUntil('|');
       Serial_input= (str.toInt()); 
       if ((Serial_input>1 && Serial_input<2200)) // its width then 
          { Step=1; }   
     } 
  if (millis()-look_out_time > 5000)    
     { look_out_var+=1;
       if (look_out_var>3) 
          { look_out_var=0 ;} 
       look_out_time=millis();  
...
{ if (Serial.available()>0)
      { String str= Serial.readStringUntil('|');
       Serial_input= (str.toInt()); 
       if (Serial_input<2200 && (Serial_input!=1 || Serial_input==0))
          {Tree_found=true; }  
       if (Serial_input>2200 && Serial_input<3000) // its width then 
          { width_tree= (width_tree*5/10)+ (Serial_input*5/10); }
       if (Serial_input>3000 && Serial_input<3600) // its center_X then 
          { center_X= (center_X*5/10)+ (Serial_input*5/10); }
       if (Serial_input==1)
          { { Move_robot(3); } 
            tree_found_surety=0;      
          }  
        ...
        if (center_X<=3350) // tree is at left then
           { Move_robot(4);}
        if (center_X>=3450) // tree is at right then
           { Move_robot(3);} 
        if (center_X>3350 && center_X<3450) // tree is almost at center then
           { if (width_tree < 2930)
                { Move_robot(1);}  
           }  
      }
